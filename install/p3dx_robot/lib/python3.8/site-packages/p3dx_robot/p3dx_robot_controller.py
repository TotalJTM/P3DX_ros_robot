...
class P3_DX_robot:
	...
	def __init__(self, nodeinst):
		...
		self.motor_update_timer = Timer(1.0/self.n.declare_parameter("freqs/update_motor", 10).value)
		self.encoder_update_timer = Timer(1.0/self.n.declare_parameter("freqs/update_encoder", 25).value)
		self.button_update_timer = Timer(1.0/self.n.declare_parameter("freqs/update_button", 10).value)

		# create timer objects that generate messages to send to arduino
		self.serial_publisher = self.n.create_publisher(String, 'serialcomm_message_out', 10)
		self.serial_publisher_timer = self.n.create_timer(1.0/self.n.declare_parameter("/serialcomm/pub_freq", 50).value, self.callback_serial_publisher)
		self.serial_subscriber = self.n.create_subscription(String, 'serialcomm_message_in', self.callback_serial_subscriber, 10)

		self.cmd_throttle_subscriber = self.n.create_subscription(Float32, 'cmd_throttle', self.callback_throttle_subscriber, 1)
		self.cmd_rotation_subscriber = self.n.create_subscription(Float32, 'cmd_rotation', self.callback_rotation_subscriber, 1)
		self.motor_mixer_timer = self.n.create_timer(1.0/self.n.declare_parameter("control/freqs/motor_mixer", 25).value, self.callback_steering_mixer)

		self.robot_reset_encoders_subscriber = self.n.create_subscription(String, 'robot_reset_encoders', self.reset_encoder_values, 1)
		self.robot_update_indicator_LED_subscriber = self.n.create_subscription(String, 'robot_update_indicator_LED', self.send_indicator_LED_values, 1)
		self.robot_update_PID_subscriber = self.n.create_subscription(String, 'robot_update_PID', self.send_pid_values, 1)
		self.robot_estop = self.n.create_service(Trigger, "robot_estop", self.estop)

		self.handle_buttons_timer = self.n.create_timer(1.0/self.n.declare_parameter("control/buttons/timer_freq", 25).value, self.handle_buttons)
	...
	def handle_buttons(self):
		...
		if self.last_motor_button_state == 0:
			...
			self.left_motor_speed = 0
			self.right_motor_speed = 0
			self.n.get_logger().info("Motor button pressed, stopping motors")
	...
	def callback_serial_publisher(self):
		#print('serial_pub')
		msg = String()

		if self.motor_update_timer.check_timer():
			msg.data = f'<10,{int(self.left_motor_speed)},{int(self.right_motor_speed)}>'
	...
	def reset_encoder_values(self, vals='{}'):
		...
		msg = String()
		msg.data = f'<13,{left_enc},{right_enc}>'
		self.serial_publisher.publish(msg)
...
def main(args=None):
	...
	node = rclpy.create_node('robotcontroller')

	#create robot object
	robot = P3_DX_robot(node)

	node.get_logger().info("P3DX maincontroller rosnode started")
	...
	node.get_logger().info("Robot fully initialized")

	rclpy.spin(node)

	node.get_logger().info("Spin exited")
